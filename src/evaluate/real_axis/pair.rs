...
use crate::{
    ...
    algebra::{ComplexJet, Jet, RealScalarAlgebra, ScalarAlgebraExpRelExt},
...
#[derive(Debug)]
pub struct RealAxisExcitationPair<'a, J, ML, MR, WL, WR>
where
...
{
    reference: RealAxisExcitation<'a, J, ML, WL>,

    comparison: RealAxisExcitation<'a, J, MR, WR>,
}

impl<'a, J, ML, MR, WL, WR> RealAxisExcitationPair<'a, J, ML, MR, WL, WR>
where
    ...
    WR: RetainedIsotropicLayers<Algebra = J>,
{
    ...
    pub fn reference(&self) -> &RealAxisExcitation<'a, J, ML, WL> {
        &self.reference
    }

    pub fn comparison(&self) -> &RealAxisExcitation<'a, J, MR, WR> {
        &self.comparison
    }
    ...
    pub fn into_parts(
        ...
        RealAxisExcitation<'a, J, ML, WL>,
        RealAxisExcitation<'a, J, MR, WR>,
    ) {
...
{
    fn layer_inputs(&self) -> Result<Layers<LayerOverlapInput<J>>, RealAxisPairError> {
        ...
        if reference_waves.len() != reference_count {
            return Err(RealAxisPairError::ReferenceWaveCountMismatch {
        ...
        if comparison_waves.len() != comparison_count {
            return Err(RealAxisPairError::ComparisonWaveCountMismatch {
        ...
        for (index, (reference_boundaries, comparison_boundaries)) in paired_waves.enumerate() {
            ...
            let reference_quantities = reference_state
                ...
                .ok_or(RealAxisPairError::MissingLayerData {
                    operand: PairOperand::Reference,
                    index: layer_index,
                    layer_count: reference_count,
                })?
    ...
    WR: ReconstructLayerBoundaryWaves<Algebra = J> + RetainedIsotropicLayers<Algebra = J>,
{
    fn hermitian_layer_inputs(
        &self,
    ) -> Result<Layers<HermitianLayerOverlapInput<J>>, RealAxisPairError> {
        self.layer_inputs()
    ...
    fn raw_layer_hermitian_overlap(
        &self,
    ) -> Result<Layers<HermitianLayerOverlap<J>>, RealAxisPairError>
    where
    ...
    fn raw_aggregate_hermitian_overlap(
        &self,
    ) -> Result<AggregateHermitianOverlap<J>, RealAxisPairError>
    where
        J: RealScalarAlgebra + ScalarAlgebraExpRelExt,
        J::Scalar: ComplexScalar,
    {
    ...
    pub fn layer_hermitian_overlap(
        &self,
    ) -> Result<DifferentialResponseFor<J, Layers<HermitianLayerOverlap<J>>>, RealAxisPairError>
    where
        J: RealScalarAlgebra + ScalarAlgebraExpRelExt,
        J::Scalar: ComplexScalar,
    ...
    pub fn aggregate_hermitian_overlap(
        &self,
    ) -> Result<DifferentialResponseFor<J, AggregateHermitianOverlap<J>>, RealAxisPairError>
    where
        J: RealScalarAlgebra + ScalarAlgebraExpRelExt,
        J::Scalar: ComplexScalar,
...
//         &self,
//     ) -> Result<Layers<BilinearLayerOverlapInput<J>>, RealAxisPairError> {
//         self.layer_inputs()
...
//         &self,
//     ) -> Result<Layers<BilinearLayerOverlap<J>>, RealAxisPairError>
//     where
...
//         &self,
//     ) -> Result<AggregateBilinearOverlap<J>, RealAxisPairError>
//     where
//         J: ScalarAlgebra + ScalarAlgebraExpRelExt,
//         J::Scalar: ComplexScalar,
//     {
...
//         &self,
//     ) -> Result<DifferentialResponseFor<J, Layers<BilinearLayerOverlap<J>>>, RealAxisPairError>
//     where
//         J: ScalarAlgebra + ScalarAlgebraExpRelExt,
//         J::Scalar: ComplexScalar,
...
//         &self,
//     ) -> Result<DifferentialResponseFor<J, AggregateBilinearOverlap<J>>, RealAxisPairError>
//     where
//         J: ScalarAlgebra + ScalarAlgebraExpRelExt,
//         J::Scalar: ComplexScalar,
...
pub(crate) fn validate_scalar_excitation_pair<J, ML, MR, WL, WR>(
    reference: &RealAxisExcitation<'_, J, ML, WL>,
    comparison: &RealAxisExcitation<'_, J, MR, WR>,
) -> Result<(), RealAxisPairError>
where
    J: Jet<Dimension = Ix0> + JetMapping + PartialEq + ComplexJet,
    J::Scalar: ComplexField,
    <J::Scalar as ComplexField>::RealField: ComplexField,
...
fn validate_polarisation<J, ML, MR, WL, WR>(
    reference: &RealAxisExcitation<'_, J, ML, WL>,
    comparison: &RealAxisExcitation<'_, J, MR, WR>,
) -> Result<(), RealAxisPairError>
where
    ...
    <J::Scalar as ComplexField>::RealField: ComplexField,
{
    ...
    if reference_polarisation != comparison_polarisation {
        return Err(RealAxisPairError::PolarisationMismatch {
...
fn validate_mapping<J, ML, MR, WL, WR>(
    reference: &RealAxisExcitation<'_, J, ML, WL>,
    comparison: &RealAxisExcitation<'_, J, MR, WR>,
) -> Result<(), RealAxisPairError>
where
    ...
    J::Mapping: PartialEq,
{
    ...
    if reference_mapping != comparison_mapping {
        return Err(RealAxisPairError::DifferentialMappingMismatch);
    }

    Ok(())
}

fn validate_layer_geometry<J, ML, MR, WL, WR>(
    reference: &RealAxisExcitation<'_, J, ML, WL>,
    comparison: &RealAxisExcitation<'_, J, MR, WR>,
) -> Result<(), RealAxisPairError>
where
    ...
    WR: RetainedIsotropicLayers<Algebra = J>,
{
    ...
    let reference_count =
        reference_workspace
            ...
            .ok_or(RealAxisPairError::LayersNotRetained {
                operand: PairOperand::Reference,
            })?;
    ...
    if reference_count != comparison_count {
        return Err(RealAxisPairError::LayerCountMismatch {
            reference_count,
            comparison_count,
        });
    }
    ...
    for index in 0..reference_count {
        ...
        let reference_thickness = reference_workspace.layer_thickness(index).ok_or(
            RealAxisPairError::MissingLayerData {
                operand: PairOperand::Reference,
                index: layer_index,
                layer_count: reference_count,
            },
        ...
        if reference_thickness != comparison_thickness {
            return Err(RealAxisPairError::LayerThicknessMismatch { index: layer_index });
        }
    }
