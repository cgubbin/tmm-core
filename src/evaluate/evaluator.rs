...
use crate::{
    ...
    backend::{Backend, HasEntries, IntoEntries},
    domain::RealAxis,
    input::{
        CompilationContext, CompileJet, CompilePlaneWaveError, Parameter, ParameterAssignment,
        ParameterAssignmentError, PlaneWaveInput, compile_real,
    },
...
impl<B> PlaneWaveEvaluator<B> {
    ...
    >
    where
        B: Backend<J, RealAxis, Workspace = W, Error = E>,
        // +PlaneWaveBackend<J>,
        W: IntoEntries,
        ...
        M: Clone + Material<Real = C::RealField>,
    {
        let (canonical_problem, context) =
            compile_real::<M, C, D, J>(input, stack, &ValidationConfig::permissive(), assignment)
