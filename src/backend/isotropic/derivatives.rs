...
pub struct IsotropicLayerFirstDerivatives<C, D>
where
    D: Dimension,
{
...
where
    D: Dimension,
{
    pub first: IsotropicLayerFirstDerivatives<C, D>,
...
where
    ...
    C: ComplexScalar,
{
    pub(crate) fn with_respect_to_vacuum_wavenumber_squared<M>(
        material: &M,
        ...
        M: Material<Real = C::RealField>,
    {
        ...
        IsotropicLayerFirstDerivatives { dkappa, dfactor }
    }

    pub(crate) fn with_respect_to_parallel_wavenumber_squared(
        q: &IsotropicLayerQuantities<C, D>,
    ) -> Self {
        let two = C::one() + C::one();
    ...
    C: ComplexScalar,
{
        ...
        M: Material<Real = C::RealField>,
    {
        ...
        // κ'' = q'' / (2κ) - (q')² / (4κ³)
        let four = two + two;
        let ddkappa = ddq / q.kappa.mapv(|k| two * k)
            - dq.mapv(|x| x * x) / q.kappa.mapv(|k| four * k * k * k);
...
mod tmm_deriv_tests {
    ...
    use crate::{
        backend::{
            PlanarInput, Polarisation, isotropic::IsotropicLayerQuantities, transfer2::Matrix2,
        },
        material::Constant,
        stack::Thickness,
    };
    ...
    fn vacuum_wavenumber_squared_second_derivative_matches_finite_difference() {
        ...
        let ddq = IsotropicLayerSecondDerivatives::with_respect_to_vacuum_wavenumber_squared(
            &material,
            &q,
            &arr0(c(vacuum_wavenumber2.sqrt())),
            Polarisation::TransverseElectric,
    ...
    fn parallel_wavenumber_squared_second_derivative_matches_finite_difference() {
        ...
        let ddq = IsotropicLayerSecondDerivatives::with_respect_to_parallel_wavenumber_squared(&q);
        let analytical = Matrix2::spectral_second_derivative(&q, thickness, &ddq);
