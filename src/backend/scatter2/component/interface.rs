...
pub(crate) fn interface<C, D, A>(left: &A, right: &A) -> Scatter2Entries<A>
where
    C: ComplexScalar,
    D: Dimension,
    A: ScalarAlgebra<C, D>,
{
    let two = A::filled_constant_like(left.value(), C::one() + C::one());
...
//     use super::*;
//     use crate::algebra::{ArrayJet1, ArrayJet2};
...
//         let left = ArrayJet1::from_parts(left, arr0(c(0.3)));
...
//         let left = ArrayJet2::from_parts(left, arr0(c(0.3)), arr0(c(0.1)));
...
//         let left = ArrayJet1::from_parts(left_value, arr0(c(0.0)));

//         let right = ArrayJet1::from_parts(right_value, arr0(c(0.0)));
