use crate::{
    ...
    observable::AggregateEnergy,
};

impl<R> IntoValue for AggregateEnergy<R>
where
    R: IntoValue,
{
    type Value = AggregateEnergy<R::Value>;

    fn into_value(self) -> ValuePart<Self::Value> {
        let (electric, magnetic, total) = self.into_parts();

        ValuePart::new(AggregateEnergy::new(
...
impl<R> IntoFirst for AggregateEnergy<R>
where
    R: IntoFirst,
{
    fn into_first(self) -> DirectionalFirstParts<Self::Value> {
        ...
        DirectionalFirstParts::new(
            AggregateEnergy::new(electric, magnetic, total),
            AggregateEnergy::new(electric_first, magnetic_first, total_first),
        )
    }
}

impl<R> IntoSecond for AggregateEnergy<R>
where
    R: IntoSecond,
{
    fn into_second(self) -> DirectionalSecondParts<Self::Value> {
        ...
        DirectionalSecondParts::new(
            AggregateEnergy::new(electric, magnetic, total),
            AggregateEnergy::new(electric_first, magnetic_first, total_first),
            AggregateEnergy::new(electric_second, magnetic_second, total_second),
        )
    }
}

impl<R> IntoBivariateFirst for AggregateEnergy<R>
where
    R: IntoBivariateFirst,
{
    fn into_bivariate_first(self) -> BivariateFirstParts<Self::Value> {
        ...
        BivariateFirstParts::new(
            AggregateEnergy::new(electric, magnetic, total),
            AggregateEnergy::new(electric_axis0, magnetic_axis0, total_axis0),
            AggregateEnergy::new(electric_axis1, magnetic_axis1, total_axis1),
        )
    }
}

impl<R> IntoBivariateSecond for AggregateEnergy<R>
where
...
mod tests {
    ...
    fn aggregate_energy(offset: f64) -> AggregateEnergy<Probe> {
        AggregateEnergy::new(probe(offset), probe(offset + 10.0), probe(offset + 20.0))
    }

    fn assert_aggregate_energy(actual: &AggregateEnergy<f64>, left: f64, right: f64, total: f64) {
