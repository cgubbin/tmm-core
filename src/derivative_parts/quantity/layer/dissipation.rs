use crate::{
    ...
    observable::LayerDissipation,
};

impl<R> IntoValue for LayerDissipation<R>
where
    R: IntoValue,
{
    type Value = LayerDissipation<R::Value>;

    fn into_value(self) -> ValuePart<Self::Value> {
        let (electric, magnetic, total) = self.into_parts();

        ValuePart::new(LayerDissipation::new(
...
impl<R> IntoFirst for LayerDissipation<R>
where
    R: IntoFirst,
{
    fn into_first(self) -> DirectionalFirstParts<Self::Value> {
        ...
        DirectionalFirstParts::new(
            ...
            LayerDissipation::new(electric_first, magnetic_first, total_first),
        )
    }
}

impl<R> IntoSecond for LayerDissipation<R>
where
    R: IntoSecond,
{
    fn into_second(self) -> DirectionalSecondParts<Self::Value> {
        ...
        DirectionalSecondParts::new(
            LayerDissipation::new(electric, magnetic, total),
            LayerDissipation::new(electric_first, magnetic_first, total_first),
            LayerDissipation::new(electric_second, magnetic_second, total_second),
        )
    }
}

impl<R> IntoBivariateFirst for LayerDissipation<R>
where
    R: IntoBivariateFirst,
{
    fn into_bivariate_first(self) -> BivariateFirstParts<Self::Value> {
        ...
        BivariateFirstParts::new(
            LayerDissipation::new(electric, magnetic, total),
            LayerDissipation::new(electric_axis0, magnetic_axis0, total_axis0),
            LayerDissipation::new(electric_axis1, magnetic_axis1, total_axis1),
        )
    }
}

impl<R> IntoBivariateSecond for LayerDissipation<R>
where
