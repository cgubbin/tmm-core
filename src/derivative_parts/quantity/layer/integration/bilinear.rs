use crate::{
    ...
    observable::AggregateBilinearNormalization,
};

impl<R> IntoValue for AggregateBilinearNormalization<R>
where
    R: IntoValue,
{
    type Value = AggregateBilinearNormalization<R::Value>;

    fn into_value(self) -> ValuePart<Self::Value> {
        let (electric, magnetic, total) = self.into_parts();

        ValuePart::new(AggregateBilinearNormalization::from_parts(
...
impl<R> IntoFirst for AggregateBilinearNormalization<R>
where
    R: IntoFirst,
{
    fn into_first(self) -> DirectionalFirstParts<Self::Value> {
        ...
        DirectionalFirstParts::new(
            AggregateBilinearNormalization::from_parts(electric, magnetic, total),
            AggregateBilinearNormalization::from_parts(electric_first, magnetic_first, total_first),
        )
    }
}

impl<R> IntoSecond for AggregateBilinearNormalization<R>
where
    R: IntoSecond,
{
    fn into_second(self) -> DirectionalSecondParts<Self::Value> {
        ...
        DirectionalSecondParts::new(
            AggregateBilinearNormalization::from_parts(electric, magnetic, total),
            AggregateBilinearNormalization::from_parts(electric_first, magnetic_first, total_first),
            AggregateBilinearNormalization::from_parts(
                electric_second,
                magnetic_second,
                total_second,
            ),
...
impl<R> IntoBivariateFirst for AggregateBilinearNormalization<R>
where
    R: IntoBivariateFirst,
{
    fn into_bivariate_first(self) -> BivariateFirstParts<Self::Value> {
        ...
        BivariateFirstParts::new(
            AggregateBilinearNormalization::from_parts(electric, magnetic, total),
            AggregateBilinearNormalization::from_parts(electric_axis0, magnetic_axis0, total_axis0),
            AggregateBilinearNormalization::from_parts(electric_axis1, magnetic_axis1, total_axis1),
        )
    }
}

impl<R> IntoBivariateSecond for AggregateBilinearNormalization<R>
where
...
mod tests {
    ...
    fn aggregate_bilinear_normalization(offset: f64) -> AggregateBilinearNormalization<Probe> {
        AggregateBilinearNormalization::from_parts(
            probe(offset),
            probe(offset + 10.0),
            probe(offset + 20.0),
    ...
    fn assert_aggregate_bilinear_normalization(
        actual: &AggregateBilinearNormalization<f64>,
        left: f64,
        right: f64,
        total: f64,
    ) {
    ...
    #[test]
    fn aggregate_bilinear_normalization_into_value_preserves_component_order() {
        let value = aggregate_bilinear_normalization(0.0)
            .into_value()
            .into_inner();

        assert_aggregate_bilinear_normalization(&value, 1.0, 11.0, 21.0);
    }

    #[test]
    fn aggregate_bilinear_normalization_into_first_preserves_all_components() {
        let (value, first) = aggregate_bilinear_normalization(0.0)
            .into_first()
            .into_parts();

        assert_aggregate_bilinear_normalization(&value, 1.0, 11.0, 21.0);

        assert_aggregate_bilinear_normalization(&first, 2.0, 12.0, 22.0);
    }

    #[test]
    fn aggregate_bilinear_normalization_into_second_preserves_all_components() {
        let (value, first, second) = aggregate_bilinear_normalization(0.0)
            .into_second()
            .into_parts();
        ...
        assert_aggregate_bilinear_normalization(&second, 4.0, 14.0, 24.0);
    }

    #[test]
    fn aggregate_bilinear_normalization_into_bivariate_first_preserves_axes() {
        let (value, axis0, axis1) = aggregate_bilinear_normalization(0.0)
            .into_bivariate_first()
            .into_parts();

        assert_aggregate_bilinear_normalization(&value, 1.0, 11.0, 21.0);

        assert_aggregate_bilinear_normalization(&axis0, 2.0, 12.0, 22.0);

        assert_aggregate_bilinear_normalization(&axis1, 3.0, 13.0, 23.0);
    }

    #[test]
    fn aggregate_bilinear_normalization_into_bivariate_second_preserves_all_branches() {
        let (value, axis0, axis1, axis0_axis0, axis0_axis1, axis1_axis1) =
            aggregate_bilinear_normalization(0.0)
        ...
        assert_aggregate_bilinear_normalization(&axis0_axis0, 4.0, 14.0, 24.0);

        assert_aggregate_bilinear_normalization(&axis0_axis1, 5.0, 15.0, 25.0);

        assert_aggregate_bilinear_normalization(&axis1_axis1, 6.0, 16.0, 26.0);
    }
}
