use crate::{
    ...
    observable::ElectromagneticEnergy,
};

impl<R> IntoValue for ElectromagneticEnergy<R>
where
    R: IntoValue,
{
    type Value = ElectromagneticEnergy<R::Value>;

    fn into_value(self) -> ValuePart<Self::Value> {
        let (electric, magnetic, total) = self.into_parts();

        ValuePart::new(ElectromagneticEnergy::new(
...
impl<R> IntoFirst for ElectromagneticEnergy<R>
where
    R: IntoFirst,
{
    fn into_first(self) -> DirectionalFirstParts<Self::Value> {
        ...
        DirectionalFirstParts::new(
            ...
            ElectromagneticEnergy::new(electric_first, magnetic_first, total_first),
        )
    }
}

impl<R> IntoSecond for ElectromagneticEnergy<R>
where
    R: IntoSecond,
{
    fn into_second(self) -> DirectionalSecondParts<Self::Value> {
        ...
        DirectionalSecondParts::new(
            ElectromagneticEnergy::new(electric, magnetic, total),
            ElectromagneticEnergy::new(electric_first, magnetic_first, total_first),
            ElectromagneticEnergy::new(electric_second, magnetic_second, total_second),
        )
    }
}

impl<R> IntoBivariateFirst for ElectromagneticEnergy<R>
where
    R: IntoBivariateFirst,
{
    fn into_bivariate_first(self) -> BivariateFirstParts<Self::Value> {
        ...
        BivariateFirstParts::new(
            ElectromagneticEnergy::new(electric, magnetic, total),
            ElectromagneticEnergy::new(electric_axis0, magnetic_axis0, total_axis0),
            ElectromagneticEnergy::new(electric_axis1, magnetic_axis1, total_axis1),
        )
    }
}

impl<R> IntoBivariateSecond for ElectromagneticEnergy<R>
where
...
mod tests {
    ...
    fn electromagnetic_energy(offset: f64) -> ElectromagneticEnergy<Probe> {
        ElectromagneticEnergy::new(probe(offset), probe(offset + 10.0), probe(offset + 20.0))
    }

    fn assert_electromagnetic_energy(
        actual: &ElectromagneticEnergy<f64>,
        left: f64,
        right: f64,
        total: f64,
    ) {
    ...
    #[test]
    fn electromagnetic_energy_into_value_preserves_component_order() {
        let value = electromagnetic_energy(0.0).into_value().into_inner();

        assert_electromagnetic_energy(&value, 1.0, 11.0, 21.0);
    }

    #[test]
    fn electromagnetic_energy_into_first_preserves_all_components() {
        let (value, first) = electromagnetic_energy(0.0).into_first().into_parts();

        assert_electromagnetic_energy(&value, 1.0, 11.0, 21.0);

        assert_electromagnetic_energy(&first, 2.0, 12.0, 22.0);
    }

    #[test]
    fn electromagnetic_energy_into_second_preserves_all_components() {
        let (value, first, second) = electromagnetic_energy(0.0).into_second().into_parts();

        assert_electromagnetic_energy(&value, 1.0, 11.0, 21.0);

        assert_electromagnetic_energy(&first, 2.0, 12.0, 22.0);

        assert_electromagnetic_energy(&second, 4.0, 14.0, 24.0);
    }

    #[test]
    fn electromagnetic_energy_into_bivariate_first_preserves_axes() {
        let (value, axis0, axis1) = electromagnetic_energy(0.0)
            .into_bivariate_first()
            .into_parts();

        assert_electromagnetic_energy(&value, 1.0, 11.0, 21.0);

        assert_electromagnetic_energy(&axis0, 2.0, 12.0, 22.0);

        assert_electromagnetic_energy(&axis1, 3.0, 13.0, 23.0);
    }

    #[test]
    fn electromagnetic_energy_into_bivariate_second_preserves_all_branches() {
        ...
        assert_electromagnetic_energy(&axis0_axis0, 4.0, 14.0, 24.0);

        assert_electromagnetic_energy(&axis0_axis1, 5.0, 15.0, 25.0);

        assert_electromagnetic_energy(&axis1_axis1, 6.0, 16.0, 26.0);
    }
}
