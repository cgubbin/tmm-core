...
pub enum SampleGenerationError<R> {
...
impl<R> FieldSampling<R> {
    ...
    pub fn positions<M>(
        &self,
        stack: &Stack<M, R>,
    ) -> Result<Vec<FieldPosition<R>>, SampleGenerationError<R>>
    where
        R: Float,
    {
            ...
            match region {
                ...
                FieldSamplingRegion::Layer { index, sampling } => {
                    let Some(layer) = stack.layers_left_to_right().get(*index) else {
                        return Err(SampleGenerationError::LayerOutOfBounds {
                            requested: *index,
...
) -> Result<(), SampleGenerationError<R>>
where
    R: Copy + Float,
{
    match sampling {
...
fn uniform_closed_interval<R>(
    ...
    points: usize,
) -> Result<Vec<R>, SampleGenerationError<R>>
where
    R: Copy + Float,
{
    if points == 0 {
        return Err(SampleGenerationError::EmptyUniformSampling);
    }
    ...
    R: Copy + Float,
{
    ...
    if points == 1 {
        return match (include_left, include_right) {
            (true, true) => Err(SampleGenerationError::AmbiguousSinglePointLayerSampling),
    ...
    R: Copy + Float,
{
    if !offset.is_finite() || offset < R::zero() || offset > thickness {
        return Err(SampleGenerationError::InvalidLayerOffset {
            layer,
            offset,
            thickness,
        });
...
pub(super) fn validate_exterior_distance<R>(distance: R) -> Result<(), SampleGenerationError<R>>
where
    R: Copy + Float,
{
    if !distance.is_finite() || distance < R::zero() {
        return Err(SampleGenerationError::InvalidExteriorDistance { distance });
    }
...
mod tests {
    ...
    fn single_sample_cannot_include_both_layer_boundaries() {
        ...
        assert_eq!(
            result,
            Err(SampleGenerationError::AmbiguousSinglePointLayerSampling,),
        );
    }
    ...
    fn zero_uniform_samples_are_rejected() {
        ...
        assert_eq!(result, Err(SampleGenerationError::EmptyUniformSampling,),);
    }
    ...
    fn negative_exterior_distance_is_rejected() {
        ...
        assert_eq!(
            error,
            SampleGenerationError::InvalidExteriorDistance { distance: -1.0 }
        );
    }
    ...
    fn non_finite_exterior_distance_is_rejected() {
        ...
        assert!(matches!(
            ...
            Err(SampleGenerationError::InvalidExteriorDistance { .. })
        ));
    }
    ...
    fn negative_layer_offset_is_rejected() {
        ...
        assert_eq!(
            error,
            SampleGenerationError::InvalidLayerOffset {
                layer: 2,
                offset: -1.0,
                thickness: 10.0,
            }
