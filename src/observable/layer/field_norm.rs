//! Integrated electromagnetic field norms in homogeneous isotropic layers.
...
/// Hermitian weights used to reconstruct the complete vector-field norm from
/// the canonical scalar state.
///
...
/// ```text
/// factor = mu       for TE
/// factor = epsilon  for TM.
/// ```
pub(crate) fn isotropic_field_norm_weights<A>(
    vacuum_angular_wavenumber: &A,
    parallel_wavenumber: &A,
    quantities: &IsotropicLayerQuantities<A>,
) -> IsotropicFieldNormWeights<A::RealJet>
where
    A: RealScalarAlgebra,
    ...
    <A::RealJet as Jet>::Scalar: One,
{
    ...
    let one = A::RealJet::filled_constant_like(
        vacuum_squared.value(),
        <A::RealJet as crate::algebra::Jet>::Scalar::one(),
    );

    let transverse = one.divide(&vacuum_squared);

    let longitudinal = parallel_wavenumber
        .divide(&vacuum_angular_wavenumber.multiply(quantities.factor()))
        .magnitude_squared();

    IsotropicFieldNormWeights::new(transverse, longitudinal)
}
...
pub(crate) struct IsotropicFieldNormWeights<A> {
    transverse: A,
    longitudinal: A,
}

impl<A> IsotropicFieldNormWeights<A> {
    pub(crate) const fn new(transverse: A, longitudinal: A) -> Self {
        Self {
            transverse,
            longitudinal,
        }
    ...
    pub(crate) fn transverse(&self) -> &A {
        &self.transverse
    }

    pub(crate) fn longitudinal(&self) -> &A {
        &self.longitudinal
    }
}
...
{
    let weights =
        isotropic_field_norm_weights(vacuum_angular_wavenumber, parallel_wavenumber, quantities);

    match quantities.polarisation() {
        Polarisation::TransverseElectric => project_te_integrated_field_norms(state, &weights),

        Polarisation::TransverseMagnetic => project_tm_integrated_field_norms(state, &weights),
    }
}

pub(crate) fn project_te_integrated_field_norms<A>(
    state: &IntegratedStateProducts<A>,
    weights: &IsotropicFieldNormWeights<A::RealJet>,
) -> IntegratedFieldNorms<A::RealJet>
where
    A: RealScalarAlgebra,
    A::RealJet: ScalarAlgebra,
{
    let field_norm = state.field_field().real();

    let secondary_norm = state.secondary_secondary().real();

    let electric = field_norm.clone();

    let magnetic = secondary_norm
        .multiply(weights.transverse())
        .add(&field_norm.multiply(weights.longitudinal()));

    IntegratedFieldNorms::new(electric, magnetic)
}

pub(crate) fn project_tm_integrated_field_norms<A>(
    state: &IntegratedStateProducts<A>,
    weights: &IsotropicFieldNormWeights<A::RealJet>,
) -> IntegratedFieldNorms<A::RealJet>
where
    ...
    A::RealJet: ScalarAlgebra,
{
    ...
    let electric = secondary_norm
        .multiply(weights.transverse())
        .add(&field_norm.multiply(weights.longitudinal()));

    let magnetic = field_norm;
